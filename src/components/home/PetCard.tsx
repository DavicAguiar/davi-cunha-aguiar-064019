...
export const PetCard: React.FC<{ pet: Pet; onEdit: () => void; onDelete: () => void }> = ({ pet, onEdit, onDelete }) => {
  return (
    <article className="group bg-white rounded-[2.5rem] border border-slate-100 p-4
     hover:shadow-2xl hover:shadow-emerald-600/10 transition-all duration-500
      hover:-translate-y-1">
      <button 
        onClick={onDelete}
        className="absolute top-6 right-6 z-10 w-10 h-10 bg-white/90
         backdrop-blur rounded-full shadow-sm text-slate-400 hover:text-red-500
          opacity-0 group-hover:opacity-100 transition-all flex items-center
           justify-center border border-slate-100 hover:scale-110 cursor-pointer"
      ...
      <div className="px-2">
        ...
        <button 
          ...
          className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black
           text-[10px] uppercase tracking-[0.2em] hover:bg-violet-600 transition-all
